...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.corrective.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.correctives.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                <label for="finding_date">{{ trans('cruds.corrective.fields.finding_date') }}</label>
                <input class="form-control date {{ $errors->has('finding_date') ? 'is-invalid' : '' }}" type="text" name="finding_date" id="finding_date" value="{{ old('finding_date') }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('sources') ? 'is-invalid' : '' }}" name="sources_id" id="sources_id">
                    @foreach($sources as $id => $sources)
                        <option value="{{ $id }}" {{ old('sources_id') == $id ? 'selected' : '' }}>{{ $sources }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('finding') ? 'is-invalid' : '' }}" name="finding" id="finding">{{ old('finding') }}</textarea>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('tanggung_jawab') ? 'is-invalid' : '' }}" name="tanggung_jawab_id" id="tanggung_jawab_id">
                    @foreach($tanggung_jawabs as $id => $tanggung_jawab)
                        <option value="{{ $id }}" {{ old('tanggung_jawab_id') == $id ? 'selected' : '' }}>{{ $tanggung_jawab }}</option>
                    @endforeach
                </select>
